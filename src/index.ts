import "./main-features/main.js";

/* ==== CSS ==== */
import "./styles/smpp-styles/dmenu.css";
import "./styles/smpp-styles/smpp.css";
import "./styles/smpp-styles/ui.css";
import "./styles/smpp-styles/games.css";
import "./styles/smpp-styles/weather.css";
import "./styles/smpp-styles/delijn.css";
import "./styles/smpp-styles/plant.css";
import "./styles/smpp-styles/clock.css";
import "./styles/smpp-styles/calendar.css";
import "./styles/smpp-styles/widgets.css";
import "./styles/smpp-styles/tutorial-widget.css";
import "./styles/smpp-styles/today.css";
import "./styles/smpp-styles/notes.css";
import "./styles/smpp-styles/focus-timer.css";
import "./styles/smpp-styles/settings.css";
import "./styles/smpp-styles/assignments.css";
import "./styles/smpp-styles/profile.css";
import "./styles/fixes/general.css";
import "./styles/fixes/login.css";
import "./styles/fixes/messages.css";
import "./styles/fixes/navigation.css";
import "./styles/fixes/notification.css";
import "./styles/fixes/planner.css";
import "./styles/fixes/results.css";
import "./styles/fixes/startpage.css";
import "./styles/fixes/agenda.css";
import "./styles/fixes/root.css";
import "./styles/fixes/smartschool-widgets.css";
